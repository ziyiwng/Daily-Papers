...
**核心总结**: 问题：Transformer推理受限于自回归解码导致中间推理状态难以持久；方法：引入T2MLR，通过融合前一标记的中间层表示到当前标记的早期层；结果：提升抽象中间推理状态的持续性。
...
**核心总结**: 问题：CPU微架构侧信道泄露信息的安全威胁；方法：提出泄漏契约作为ISA级安全抽象；结果：因微架构复杂性难以制定有效泄漏契约。
...
**核心总结**: 问题：如何将拓扑空间理论推广到任意范畴上的余单子；方法：通过密度余单子抽象子基，研究子基、基及连续映射；结果：定义了余单子间的连续映射并恢复常规拓扑概念。
...
**核心总结**: 问题：年轻岩质行星大气受初始挥发物与逃逸竞争影响；方法：扩展耦合地层-大气模型模拟熔岩海洋结晶期间大气演化；结果：无。
...
**核心总结**: 问题：验证DPLL算法的抽象状态转换系统；方法：在Rocq证明助手中原型化命题公式语法与语义，定义经典和基础DPLL转换系统；结果：证明其关键元理论性质。
...
**核心总结**: 问题：对称二阶线性椭圆PDE的自适应有限元方法；方法：基于后验误差估计器和迭代求解器的自适应算法；结果：证明算法在任意用户选择的自适应参数下均实现无条件全R线性收敛。
...
**核心总结**: 问题：现有自动驾驶车辆缺乏高效协同机制，仅共享低层次运动状态；方法：提出基于边缘计算的高阶意图通信框架；结果：实现车辆间决策仲裁与协同控制，提升整体交通效率。
...
**核心总结**: 问题：确定单次遍历半流模型中最大匹配问题的最佳近似比；方法：研究单次遍历半流算法的性能；结果：已知存在0.5-近似算法，但最佳比尚未确定。
...
**核心总结**: 问题：奇数股 Turk's head 结的等变 Q-可解性和 Klein 反向性；方法：通过提升符号箭头高斯图的对称性构造交换的环境对合；结果：当 q 为奇数时，结为 Klein 反向且等变 Q-可解；当 q 为偶数时，未给出具体结果。
