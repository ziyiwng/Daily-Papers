...
**核心总结**: 问题：评估大语言模型文本输出的质量和相关性困难且资源消耗大；方法：提出一种无需显式参考标准的新型评估框架；结果：可近似专家级评估，适用于无明确基准的领域。
...
**核心总结**: 问题：现有记忆增强型语言模型代理在检索经验时存在与当前情境不匹配的问题；方法：提出动态适配机制，根据当前状态调整检索内容；结果：无。
...
**核心总结**: 问题：现有视觉推理方法受限于外部定义的推理轨迹和视觉操作，缺乏灵活性；方法：引入潜在空间中的推理机制，将中间计算内化为连续表示；结果：提升视觉推理的抽象性和灵活性。
...
**核心总结**: 问题：现有遥感多时相变化检测方法无法有效表征反复变化区域；方法：提出Urban Building Dynamics Detection（UBD）模型，捕捉城市建筑动态变化；结果：可更准确识别与人类活动相关的重复变化区域。
